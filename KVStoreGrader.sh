...
then
	...
	do
		...
		echo "<<<<<<<<<<<<<<<<"
    echo $fail
    echo $time_of_this_fail
    echo $read_op_test3_part1_time
    # if [ "${time_of_this_fail}" -ge "${read_op_test3_part1_time}" ]
      ...
      echo $actual_key
      echo $read_test3_part1_fail_count
			if [ "${actual_key}"  -eq 1 ]
			then	
